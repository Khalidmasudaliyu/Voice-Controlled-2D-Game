...
class VoiceLearningApp(tk.Tk):
    def __init__(self):
        ...
        self.title_label = tk.Label(
            top, text=t("welcome"), font=TITLE_FONT, bg=HEADER_BG, fg="white"
        )
        ...
        self.lang_btn = tk.Button(
            right,
            text=f"{t('language')}: {current_language().upper()}",
            command=self.toggle_language,
        )
        self.lang_btn.pack(side="left", padx=6)
        self.test_btn = tk.Button(
            right, text=t("test_voice"), command=lambda: speak(t("welcome"))
        )
    ...
    def show_login_screen(self):
        ...
        tk.Label(
            frame, text=t("login"), font=("Comic Sans MS", 24, "bold"), bg=BG
        ).pack(pady=10)

        tk.Label(frame, text=t("enter_name"), font=("Segoe UI", 13), bg=BG).pack()
        name_entry = tk.Entry(frame, font=("Segoe UI", 18), width=24)
        name_entry.pack(pady=(2, 8))
        tk.Label(frame, text=t("enter_pin"), font=("Segoe UI", 13), bg=BG).pack()
        pin_entry = tk.Entry(frame, font=("Segoe UI", 18), show="*", width=24)
        pin_entry.pack(pady=(2, 6))
        ...
        tk.Button(
            ...
            bg="#6fe7d6",
            font=BTN_FONT,
            command=lambda: self.do_login(name_entry.get(), pin_entry.get()),
    ...
    def do_login(self, name, pin):
        ...
        messagebox.showinfo(
            t("login"), t("welcome_back").format(name=self.current_player_name)
        )
        self.show_main_menu()
    ...
    def show_register_screen(self):
        ...
        frame.pack(expand=True)
        tk.Label(
            frame, text=t("register"), font=("Comic Sans MS", 24, "bold"), bg=BG
        ).pack(pady=10)
        tk.Label(frame, text=t("enter_name"), font=("Segoe UI", 13), bg=BG).pack()
        name_entry = tk.Entry(frame, font=("Segoe UI", 18), width=24)
        name_entry.pack(pady=(2, 8))
        tk.Label(frame, text=t("enter_pin"), font=("Segoe UI", 13), bg=BG).pack()
        pin_entry = tk.Entry(frame, font=("Segoe UI", 18), show="*", width=24)
        pin_entry.pack(pady=(2, 8))
        tk.Label(frame, text=t("confirm_pin"), font=("Segoe UI", 13), bg=BG).pack()
        confirm_entry = tk.Entry(frame, font=("Segoe UI", 18), show="*", width=24)
        confirm_entry.pack(pady=(2, 8))
        tk.Label(frame, text=t("enter_age"), font=("Segoe UI", 13), bg=BG).pack()
        age_entry = tk.Entry(frame, font=("Segoe UI", 18), width=24)
        age_entry.pack(pady=(2, 6))
        ...
        tk.Button(
            ...
            command=lambda: self.do_register(
                name_entry.get(), pin_entry.get(), confirm_entry.get(), age_entry.get()
            ),
        ).pack(pady=10)
        tk.Button(
            frame,
    ...
    def do_register(self, name, pin, confirm_pin, age_text):
        ...
        update_last_login(self.current_player_id)
        messagebox.showinfo(
            t("register"), t("welcome_back").format(name=self.current_player_name)
        )
        self.show_main_menu()
    ...
    def show_main_menu(self):
        ...
        make_btn(
            "single_player", self.enter_single_name, bg="#ffd86b", imgname="star.png"
        ).grid(row=0, column=0, padx=10, pady=8)
        make_btn(
            "multiplayer", self.enter_multi_names, bg="#9ad0f5", imgname="trophy.png"
        ).grid(row=0, column=1, padx=10, pady=8)
        make_btn(
            "sentence_game", self.enter_sentence_mode, bg="#b2f7ef", imgname="home.png"
        ).grid(row=0, column=2, padx=10, pady=8)
        make_btn(
            "view_scores", self.show_scores, bg="#b2f7ef", imgname="scores.png"
        ...
        make_btn("quit", self.quit, bg="#ffb703", imgname="quit.png").grid(
    ...
    def enter_single_name(self):
        ...
        tk.Button(
            ...
            bg="#6fe7d6",
            font=BTN_FONT,
            command=lambda: self.start_single(e.get()),
        ).pack(pady=10)
        tk.Button(f, text=t("back"), bg="#ff9aa2", command=self.show_main_menu).pack(
            pady=6
        )
    ...
    def enter_multi_names(self):
        ...
        tk.Button(
            ...
            bg="#ffd86b",
            font=BTN_FONT,
            command=lambda: self.start_multi(p1.get(), p2.get()),
        ).pack(pady=10)
        tk.Button(f, text=t("back"), bg="#ff9aa2", command=self.show_main_menu).pack(
            pady=6
        )
    ...
    def start_multi(self, n1, n2):
        if not n1.strip() or not n2.strip():
            messagebox.showinfo("Info", t("name_required"))
            return
        self.player1 = n1.strip()
        self.player2 = n2.strip()
    ...
    def choose_game_mode(self):
        ...
        f.pack(expand=True)
        tk.Label(
            f, text=t("choose_game_mode"), font=("Comic Sans MS", 22, "bold"), bg=BG
        ).pack(pady=12)
        bf = tk.Frame(f, bg=BG)
        bf.pack(pady=8)
        tk.Button(
            bf,
            text=t("alphabet_game"),
            bg="#ff9aa2",
            font=BTN_FONT,
            width=22,
            command=lambda: self.choose_difficulty("alphabet"),
        ...
        tk.Button(
            ...
            font=BTN_FONT,
            width=22,
            command=lambda: self.choose_difficulty("numbers"),
    ...
    def choose_difficulty(self, game_type):
        ...
        f.pack(expand=True)
        tk.Label(
            f, text=t("choose_difficulty"), font=("Comic Sans MS", 20), bg=BG
        ).pack(pady=12)
        bf = tk.Frame(f, bg=BG)
        bf.pack(pady=8)
        tk.Button(
        ...
        tk.Button(f, text=t("back"), bg="#ffd86b", command=self.choose_game_mode).pack(
            pady=14
        )
    ...
    def show_round_screen(self):
        ...
        self.prompt_label = tk.Label(
            frame, text="", font=("Segoe UI", 36, "bold"), bg=BG, fg="#333"
        )
        ...
        self.tries_label.pack(pady=6)
        self.feedback_label = tk.Label(frame, text="", font=("Segoe UI", 14), bg=BG)
        self.feedback_label.pack(pady=6)
        self.objects_frame = tk.Frame(frame, bg=BG)
        self.objects_frame.pack(pady=8)
        control_frame = tk.Frame(frame, bg=BG)
        ...
        score_frame = tk.Frame(frame, bg=BG)
        ...
        if self.single_mode:
            ...
            self.score_label = tk.Label(
                score_frame,
                text=f"{self.player1}: {self.session_scores['player1']}  |  {self.player2}: {self.session_scores['player2']}",
                font=("Segoe UI", 15, "bold"),
                bg=BG,
                fg="#333",
            )
    ...
    def generate_question(self):
        for w in self.objects_frame.winfo_children():
            w.destroy()
        gm = self.game_type
        d = self.difficulty
        if gm == "alphabet":
            ...
            for i in range(n):
                c = tk.Canvas(
                    self.objects_frame, width=36, height=36, bg=BG, highlightthickness=0
                )
            ...
            if not self.single_mode:
                ...
                self.prompt_label.config(text=f"{playername}: {t('say_number')} {n}")
            ...
            if not self.single_mode:
                ...
                self.prompt_label.config(text=f"{playername}: {qtext}")
                speak(f"{playername}, {speak_text}")
            else:
                self.prompt_label.config(text=qtext)
                speak(speak_text)
        self.tries_left = TRIES_PER_QUESTIONS
        self.tries_label.config(text=t("tries_left").format(tries=self.tries_left))
        self.feedback_label.config(text="")
    ...
    def on_space(self, event):
        if (
            hasattr(self, "speak_btn")
            and self.speak_btn
            and self.speak_btn.winfo_exists()
        ):
            self.on_speak()
    ...
    def _on_listen_result(self, listen_id, result_text):
        def _proc():
            ...
            if result_text == ERR_NO_MIC:
                safe_config(
                    self.feedback_label,
                    text="⚠️ No microphone found. Check your mic and try again.",
                    fg="#cc0000",
                )
                return
            if result_text == ERR_NO_INTERNET:
                safe_config(
                    self.feedback_label,
                    text="⚠️ No internet connection for speech recognition.",
                    fg="#cc0000",
                )
                return
            if result_text == ERR_NO_SPEECH:
                safe_config(
                    self.feedback_label,
                    text="🔇 Nothing heard — speak louder and try again.",
                    fg="#cc6600",
                )
                return
    ...
    def process_voice_result(self, result_text):
        ...
        if ok:
            ...
            if self.tries_left > 0:
                ...
                safe_config(
                    self.feedback_label,
                    text=t("incorrect_move").format(ans=self.current_answer)
                    + heard_msg,
                )
    ...
    def finish_round_single(self):
        ...
        f.pack(expand=True)
        tk.Label(
            f, text=t("game_over"), font=("Comic Sans MS", 22, "bold"), bg=BG
        ).pack(pady=8)
        tk.Label(
            f,
            text=t("round_finished_single").format(
                player=self.player_single, score=score
            ),
            font=("Segoe UI", 16),
            bg=BG,
        ).pack(pady=6)
        ...
        tk.Button(
            f, text=t("back"), bg="#ff9aa2", font=BTN_FONT, command=self.show_main_menu
        ).pack(pady=6)
    ...
    def finish_round_multi(self):
        ...
        f = tk.Frame(self.container, bg=BG)
        f.pack(expand=True)
        p1score = self.session_scores.get("player1", 0)
        p2score = self.session_scores.get("player2", 0)
        tk.Label(
            f, text=t("game_over"), font=("Comic Sans MS", 22, "bold"), bg=BG
        ).pack(pady=8)
        tk.Label(
            f,
            text=t("round_finished_multi").format(
                p1=self.player1, s1=p1score, p2=self.player2, s2=p2score
            ),
            font=("Segoe UI", 16),
            bg=BG,
        ).pack(pady=6)
        ...
        tk.Label(
            f,
            text=t("match_finished").format(winner=winner),
    ...
    def show_scores(self):
        ...
        tk.Label(
            f, text=t("view_scores"), font=("Comic Sans MS", 20, "bold"), bg=BG
        ).pack(pady=8)
        tk.Label(
            f, text=t("single_player"), font=("Segoe UI", 16, "underline"), bg=BG
        ).pack(pady=6)
        ...
        if single:
            for name, pts in single.items():
                ...
                stars = "⭐" * (pts // 5)
                tk.Label(sf, text=stars, bg=BG).pack(side="left", padx=8)
        else:
            tk.Label(f, text=t("no_scores"), bg=BG).pack()
        tk.Label(
            f, text=t("multiplayer"), font=("Segoe UI", 16, "underline"), bg=BG
        ).pack(pady=10)
        multi = data.get("multi", {})
        if multi:
            for match_key, hist in multi.items():
                mf = tk.Frame(f, bg=BG, bd=1, relief="solid")
                mf.pack(fill="x", padx=16, pady=6)
                tk.Label(mf, text=match_key, font=("Segoe UI", 14, "bold"), bg=BG).pack(
                    anchor="w", padx=8, pady=4
                )
                if isinstance(hist, list):
                    for entry in hist[-10:]:
                        tk.Label(mf, text=entry, font=("Segoe UI", 12), bg=BG).pack(
                            anchor="w", padx=12
                        )
                else:
                    tk.Label(mf, text=str(hist), font=("Segoe UI", 12), bg=BG).pack(
                        anchor="w", padx=12
                    )
                tk.Button(
                    mf,
                    ...
                    bg="#d62828",
                    fg="white",
                    command=lambda k=match_key: self.confirm_delete_pair(k),
        ...
        tk.Button(
            ...
            text=t("reset_scores"),
            bg="#ff9aa2",
            command=self.reset_scores_confirm,
        ).pack(side="left", padx=8)
